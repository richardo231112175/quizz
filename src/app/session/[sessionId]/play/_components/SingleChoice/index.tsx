...
export default function SingleChoice({ options, answer, setAnswer, correctAnswer, isSubmitting }: SingleChoiceProps): JSX.Element {
    return (
        <div className="space-y-3">
            {options.map((option, index) => {
                const isSelected: boolean = option === answer;
                const className: string = correctAnswer ? (
                ...
                return (
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            {isSelected && correctAnswer && option === correctAnswer && <CheckCircle className="h-5 w-5 text-green-600" />}
                            {isSelected && correctAnswer && option !== correctAnswer && <XCircle className="h-5 w-5 text-red-600" />}
                        </div>
                    </motion.button>
                );
            })}
